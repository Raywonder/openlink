...
- **openlink_monitor**: Optional local device worker
  - Tracks local connection attempts when enabled by the user or administrator
  - Logs local session activity needed for troubleshooting and security review
  - Reports to the server hub when gateway monitoring is available
  - Must not be required for normal OpenLink or OpenClaw hosting
...
### Implementation
```javascript
// update-helper.js - Native updater helper
class UpdateHelper {
...
- Automatically runs PowerShell commands to (idempotent + debug friendly):
  ```powershell
  ...
  # Ensure service exists, starts now, and starts on boot
  if (Get-Service sshd -ErrorAction SilentlyContinue) {
    Set-Service -Name sshd -StartupType Automatic
    Start-Service sshd
  } else {
    throw "OpenSSH Server service (sshd) not found after capability install"
  }

  # Ensure firewall rule exists without failing on duplicates
...
#### 1.5 WSL + Headscale/Tailscale SSH Baseline
- For WSL-only nodes, run user-mode `sshd` when root service management is unavailable.
- Default to key auth only and expose a non-privileged port (example: `2222`).
- Bind to `0.0.0.0`/`::` so the service is reachable on the WSL Tailscale/Headscale address.
- Store config and host keys in user space for portable setup:
  - `~/.config/wsl-ssh/sshd_config`
  - `~/.config/wsl-ssh/ssh_host_ed25519_key`
- Startup command:
  - `/usr/sbin/sshd -f ~/.config/wsl-ssh/sshd_config`
- Verify:
  - `ss -tln | grep 2222`
  - `ssh -p 2222 <user>@<tailscale-or-headscale-ip>`

#### 1.6 Validated Tailnet Access (March 14, 2026)
- Windows host (`DOM-PC-LAPTOP`) tailnet IPv4: `100.64.0.5`
- WSL has no separate tailnet IP in current setup; access WSL through Windows host.
- Windows `portproxy` forwards `0.0.0.0:2222 -> 127.0.0.1:2222` to reach WSL user-mode `sshd`.
- Confirmed Windows listener state:
  - `0.0.0.0:22` (system OpenSSH endpoint on host)
  - `0.0.0.0:2222` (forwarder to WSL `sshd`)
- Mac client commands with `raywonder` key:
  - Host OpenSSH: `ssh -i ~/.ssh/raywonder dom@100.64.0.5`
  - WSL OpenSSH via forward: `ssh -i ~/.ssh/raywonder -p 2222 dom@100.64.0.5`
