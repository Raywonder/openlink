...
class OpenLinkSignalingServerV2 {
    constructor(options = {}) {
        ...
        this.machineIndex = new Map(); // canonical machine id/name -> connectionId
    ...
    setupAPIRoutes() {
        ...
        this.app.get('/diagnostics/status', (req, res) => {
            res.json({
                status: 'ok',
                version: OPENLINK_SIGNALING_VERSION,
                connections: this.connections.size,
        ...
        // Storage paths info - shows where files are stored for the account
        this.app.get("/api/v2/storage", (req, res) => {
            const os = require("os");
            const username = process.env.USER || process.env.USERNAME || "unknown";
            const homeDir = os.homedir();
            const isRoot = process.getuid && process.getuid() === 0;
            
            // Define storage paths based on user context
            const storagePaths = {
                user: username,
                isRoot: isRoot,
                paths: {
                    audio: isRoot ? "/var/lib/openlink/audio" : path.join(homeDir, ".openlink", "audio"),
                    files: isRoot ? "/var/lib/openlink/files" : path.join(homeDir, ".openlink", "files"),
                    recordings: isRoot ? "/var/lib/openlink/recordings" : path.join(homeDir, ".openlink", "recordings"),
                    temp: isRoot ? "/tmp/openlink-uploads" : path.join(os.tmpdir(), "openlink-uploads"),
                    config: isRoot ? "/etc/openlink" : path.join(homeDir, ".openlink", "config"),
                    logs: isRoot ? "/var/log/openlink" : path.join(homeDir, ".openlink", "logs"),
                    web3dns: path.join(homeDir, ".openlink", "web3dns")
                },
                limits: {
                    maxFileSize: "100MB",
                    maxAudioDuration: "24h",
                    retentionDays: 30
                }
            };
        ...
        this.app.get("/", (req, res) => {
            ...
            if (isBrowser) {
                // Serve HTML status page for browsers
...
<head>
    ...
    <title>OpenLink Signaling Server</title>
    <style>
        body {
        ...
        h1 { color: #4da6ff; margin-bottom: 10px; }
        .status { 
        ...
        .download-btn:hover { background: #3d8bd9; }
        .version { color: #666; margin-top: 20px; font-size: 0.85em; }
    </style>
</head>
<body>
    <div class="container">
        <h1>OpenLink</h1>
        <p>Accessible Remote Desktop</p>
        ...
        <a href="https://raywonderis.me/apps/openlink" class="download-btn">Download OpenLink</a>
        <p class="version">Signaling Server ${OPENLINK_SIGNALING_VERSION}</p>
    </div>
</body>
</html>`;
        ...
        // =====================================================
        // DOMAINS API - For client apps to fetch available domains
        // =====================================================
        
        // Get list of validated domains for link generation
        this.app.get('/api/domains', async (req, res) => {
            try {
                // Read domains.json config
                const domainsPath = path.join(__dirname, 'domains.json');
                let domainsConfig = {
                    domains: [
                        { domain: 'openlink.tappedin.fm', active: true, primary: true },
                        { domain: 'openlink.raywonderis.me', active: true },
                        { domain: 'openlink.devinecreations.net', active: true }
                    ],
                ...
                // Return domains (active ones only)
                let validatedDomains = domainsConfig.domains.filter(d => d.active !== false);
                
                res.json({
                    success: true,
                    domains: validatedDomains,
                    primary: validatedDomains.find(d => d.primary)?.domain || validatedDomains[0]?.domain,
                    fallbackToPublicIp: domainsConfig.fallbackToPublicIp,
                ...
                res.status(500).json({ 
                    ...
                    error: error.message,
                    domains: [
                        { domain: 'openlink.tappedin.fm', primary: true }
                    ]
                });
            }
        });
        
        // Device management API - Get all devices for a wallet/user
        this.app.get('/api/devices', (req, res) => {
            const devices = [];
            
            // Get all hosting sessions
            this.sessions.forEach((session, id) => {
                if (session.host) {
                    devices.push({
                        sessionId: id,
                        deviceName: session.deviceName || session.hostname || id,
        ...
        this.app.post('/api/devices/:sessionId/nickname', (req, res) => {
        ...
        // Remove device/session link
        this.app.delete('/api/devices/:sessionId', (req, res) => {
            const { sessionId } = req.params;
        ...
        // Telemetry endpoints
        this.telemetryLog = [];
        this.telemetryFile = path.join(__dirname, "telemetry", "events.jsonl");

        // Report error/event from client
        this.app.post("/api/telemetry/report", (req, res) => {
            const { type, message, stack, deviceId, deviceName, platform, version, sessionId, metadata } = req.body;
            const event = {
                type: type || "error",
                message,
                stack,
            ...
            // Store in memory (last 1000 events)
            ...
            // Append to file
            try {
                fs.appendFileSync(this.telemetryFile, JSON.stringify(event) + "\n");
            } catch (e) {
                console.error("Failed to write telemetry:", e.message);
            }
            
            console.log(`[Telemetry] ${type}: ${message} from ${deviceName || deviceId || "unknown"}`);
            res.json({ success: true });
        });

        // Get telemetry events (for admin viewing)
        this.app.get("/api/telemetry/events", (req, res) => {
            const limit = parseInt(req.query.limit) || 100;
            const type = req.query.type; // Filter by type
            let events = this.telemetryLog.slice(-limit);
            if (type) {
                events = events.filter(e => e.type === type);
            }
            res.json({
                total: this.telemetryLog.length,
                events: events.reverse()
            });
        });

        // Clear telemetry (admin)
        this.app.delete("/api/telemetry/events", (req, res) => {
            this.telemetryLog = [];
    ...
    destroySession(sessionId) {
        ...
        session.connections.forEach(connectionId => {
            ...
            this.unregisterMachine(connectionId);
    ...
    async handleWebSocketMessage(connectionId, data) {
        ...
        switch (type) {
            ...
            case 'handshake':
                await this.handleMachinePresence(connectionId, data);
                break;

            case 'broadcast':
                await this.handleBroadcastMessage(connectionId, data);
                break;
            ...
            case 'diagnostic_event':
                await this.handleDiagnosticEventMessage(connectionId, data);
                break;
    ...
    async handleCreateSession(connectionId, data) {
        ...
        const { linkId, sessionId, password, hostInfo, machineInfo, settings } = data;
        const effectiveHostInfo = machineInfo || hostInfo || {};
        ...
        connection.machineId = effectiveHostInfo?.machineId || effectiveHostInfo?.id || connection.machineId;
        connection.machineName = effectiveHostInfo?.machineName || effectiveHostInfo?.displayName || connection.machineName;
        this.registerMachine(connectionId, effectiveHostInfo);
        connection.machineId = effectiveHostInfo?.machineId || effectiveHostInfo?.id || connection.machineId;
        connection.machineName = effectiveHostInfo?.machineName || effectiveHostInfo?.displayName || connection.machineName;
        this.registerMachine(connectionId, effectiveHostInfo);
    ...
    async handleHostSession(connectionId, data) {
        ...
        const { linkId, sessionId, password, hostInfo, machineInfo } = data;
        const effectiveHostInfo = machineInfo || hostInfo || {};
        ...
        if (!session) {
            ...
            session.hostInfo = Object.keys(effectiveHostInfo).length ? effectiveHostInfo : session.hostInfo;
        }
    ...
    async handleJoinByLinkId(connectionId, data) {
        ...
        this.registerMachine(connectionId, data.machineInfo || clientInfo || {});
    ...
    findConnectionByMachineId(machineId) {
        const token = this.canonicalMachineToken(machineId);
        const indexedConnectionId = token ? this.machineIndex.get(token) : null;
        ...
        for (const [id, conn] of this.connections.entries()) {
            ...
            if (candidates.some(candidate => this.canonicalMachineToken(candidate) === token)) {
                this.registerMachine(id, conn);
                return conn;
            }
        }
        return null;
    ...
    async handleMachinePresence(connectionId, data) {
        ...
        const machineInfo = data.machineInfo || data.hostInfo || data.clientInfo || {};
        this.registerMachine(connectionId, machineInfo);
        connection.lastActivity = new Date().toISOString();
        this.writeControlDiagnostic('machine_presence', connectionId, {
            machineId: machineInfo.id || machineInfo.machineId || connection.machineId,
            machineName: machineInfo.displayName || machineInfo.machineName || connection.machineName,
            aliases: Array.isArray(machineInfo.aliases) ? machineInfo.aliases : []
        });
        connection.ws.send(JSON.stringify({
            type: data.type === 'host_session' || data.type === 'host-session' ? 'host_session_ok' : 'handshake_ack',
            clientId: connectionId,
            connectionId,
            machineId: machineInfo.id || machineInfo.machineId || connection.machineId || connectionId,
            timestamp: Date.now()
        }));
    }
    ...
    async handleBroadcastMessage(connectionId, data) {
        ...
        const message = { type: 'broadcast', data: payload, fromId: connectionId, fromConnectionId: connectionId };
        if (payload.targetMachineId) {
            const targetConnection = this.findConnectionByMachineId(payload.targetMachineId);
            if (targetConnection?.ws) {
                this.writeControlDiagnostic(payload.type || 'targeted_broadcast', connectionId, {
            ...
            connection.ws.send(JSON.stringify({
                type: 'error',
                error: 'Target machine not found or offline',
                requestType: payload.type || data.type,
                requestId: payload.requestId,
                targetMachineId: payload.targetMachineId,
                timestamp: Date.now()
            }));
        ...
        if (connection.sessionId) {
            this.broadcastToSession(connection.sessionId, message, connectionId);
        }
    }

    async handleTargetedControlResponse(connectionId, data) {
        const connection = this.connections.get(connectionId);
        if (!connection) return;

        connection.lastActivity = new Date().toISOString();
        const targetMachineId = data.targetMachineId || data.controllerMachineId || data.toMachineId;
        const type = data.type || 'targeted_control_response';
        if (!targetMachineId) {
            this.writeControlDiagnostic(type, connectionId, {
    ...
    async handleDiagnosticEventMessage(connectionId, data) {
        const connection = this.connections.get(connectionId);
        if (!connection) return;
        connection.lastActivity = new Date().toISOString();
        this.writeControlDiagnostic(data.eventName || 'diagnostic_event', connectionId, {
            targetMachineId: data.targetMachineId,
            requestId: data.requestId,
            outcome: data.outcome,
            metadata: data.metadata || {}
        });
        connection.ws.send(JSON.stringify({ type: 'diagnostic_event_ack', eventName: data.eventName || 'event', timestamp: Date.now() }));
    }

    writeControlDiagnostic(eventName, connectionId, details = {}) {
        try {
            fs.mkdirSync(CONTROL_DIAGNOSTICS_DIR, { recursive: true });
            const connection = this.connections.get(connectionId) || {};
            const entry = {
                timestamp: new Date().toISOString(),
                eventName,
                connectionId,
                sourceMachineId: connection.machineId,
                sourceMachineName: connection.machineName || connection.displayName || connection.hostname,
                sourcePlatform: connection.platform || connection.os,
                ...details
            };
            fs.appendFileSync(CONTROL_DIAGNOSTICS_FILE, `${JSON.stringify(entry)}\n`);
        } catch (error) {
            console.warn(`[Diagnostics] Could not write control event: ${error.message}`);
        }
    }


    registerMachine(connectionId, machineInfo = {}) {
        const connection = this.connections.get(connectionId);
        if (!connection) return;
        ...
        const candidates = [
            ...
            machineInfo.displayName,
            connection.machineId,
            connection.machineName,
            connection.hostname,
            connection.displayName,
            ...(Array.isArray(machineInfo.aliases) ? machineInfo.aliases : [])
        ];

        connection.machineId = machineInfo.machineId || machineInfo.id || connection.machineId || connectionId;
        ...
        for (const candidate of candidates) {
            ...
            if (token) this.machineIndex.set(token, connectionId);
        }
    }

    unregisterMachine(connectionId) {
        for (const [token, indexedConnectionId] of this.machineIndex.entries()) {
            if (indexedConnectionId === connectionId) this.machineIndex.delete(token);
        }
    }

    canonicalMachineToken(value) {
    ...
    handleClientInfo(connectionId, payload) {
        ...
        this.registerMachine(connectionId, payload);
    ...
    broadcastToSession(sessionId, message, excludeConnectionId = null) {
        ...
        const connectionIds = Array.isArray(session.connections)
            ? session.connections
            : [
                session.host,
    ...
    cleanupConnection(connectionId) {
        ...
        if (connection.sessionId) {
            ...
            if (session) {
                ...
                if (session.host === connectionId) {
                    session.host = null;
                }
                if (session.clients?.delete) {
                ...
                const hasLegacyConnections = Array.isArray(session.connections) && session.connections.length > 0;
                const hasModernConnections = !!session.host || (session.clients?.size || 0) > 0;
                if (!hasLegacyConnections && !hasModernConnections) {
...
if (require.main === module) {
    ...
    const server = new OpenLinkSignalingServerV2({
        port,
        enableDomains: process.env.OPENLINK_ENABLE_DOMAINS !== 'false',
        enableCLI: process.env.OPENLINK_ENABLE_CLI !== 'false'
    });
